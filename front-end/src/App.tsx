...
import type { ChainId, Portfolio, Protocol, TxState, Toast, FeedItem, HistoryEntry, Scenario, DAppContextType } from '@types'
...
function seedFeedFor(chainId: ChainId): FeedItem[] {
  const addrs: Record<ChainId, string[]> = {
...
export function App() {
  const [theme, setTheme] = useState<'dark' | 'light'>(
    () => (localStorage.getItem('verda.theme') as 'dark' | 'light') ?? 'dark'
  )
  const [chain, setChainState] = useState<ChainId>(
    () => (localStorage.getItem('verda.chain') as ChainId) ?? 'ethereum'
  )
  ...
  const [wallet, setWallet] = useState<DAppContextType['wallet']>(() => ({ status: walletService.getState().status }))
  ...
  const [pf, setPf] = useState<Record<ChainId, Portfolio>>(() => ({
    ethereum: { invested: 2.5, returns: 0.34 },
    solana:   { invested: 96.0, returns: 12.4 },
  }))
  const [proto, setProto] = useState<Record<ChainId, Protocol>>(() => ({
  ...
  const [scenarios, setScenarios] = useState<Scenario>({ reject: false, insufficient: false, contract: false })

  const [toasts, setToasts] = useState<Toast[]>([])
  const pushToast = useCallback((msg: string, kind: 'brand' | 'pos' = 'brand', ttl = 3200) => {
    const id = Math.random().toString(36).slice(2)
    setToasts(t => [...t, { id, msg, kind }])
    setTimeout(() => setToasts(t => t.filter(x => x.id !== id)), ttl)
  }, [])

  const [feedMap, setFeedMap] = useState<Record<ChainId, FeedItem[]>>(() => ({
    ethereum: seedFeedFor('ethereum'),
    solana:   seedFeedFor('solana'),
  }))
  const [historyMap, setHistoryMap] = useState<Record<ChainId, HistoryEntry[]>>({ ethereum: [], solana: [] })
  ...
  const flashStat = useCallback((key: string) => {
    setStatFlash(key)
  ...
  const addFeed = useCallback((item: Omit<FeedItem, 'id' | 'label' | 'time' | 'fresh'>) => {
    const entry: FeedItem = { ...item, id: Math.random().toString(36).slice(2), label: 'InvestmentMade', time: 'just now', fresh: true }
  ...
  const ctx: DAppContextType = {
    ...
    scenarios, toggleScenario: (k: keyof Scenario) => setScenarios(s => ({ ...s, [k]: !s[k] })),
    toasts, feed, history,
  }

  return (
    <DAppProvider value={ctx}>
      <Header />
      <main className="app">
        <div className="shell">
          <PageHead />
